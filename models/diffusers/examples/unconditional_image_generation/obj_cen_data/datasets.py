...
from .types import DataDict, FeatureMetadata, MetadataDict
...
class MultiObjectDataset(Dataset):
    ...
    def __post_init__(self):
        ...
        self.full_dataset_path = self.dataset_path
        ...
        for feature in self.output_features:
            # if feature != 'image':
            #     continue
            self.data[feature] = self.dataset[feature][:self.dataset_size]
    ...
    def __getitem__(self, idx):
        out = {}
        # out['input'] = self.data['image'][idx]
        out['input'] = self.transform(
        ...
        # out['input'] = self.transform(out['image'])
...
def _load_data_hdf5(
    ...
    metadata_fname = f"{data_path.split('-')[0]}-{metadata_suffix}"
...
class MSN_Easy(Dataset):
    def __init__(self, root, img_size, num_segs, transform, transform_mask=None, split='train'):
        self.split=split
...
class PTR(Dataset):
    def __init__(self, root, img_size, num_segs, transform, transform_mask=None, split='train'):
        self.split=split
        self.root = os.path.join(root, split, 'images')
        print(self.root)
        self.img_size = img_size
        # import ipdb; ipdb.set_trace(context=15)
        self.total_imgs = [x for x in Path(self.root).glob('*.png')]
        self.num_segs = num_segs
        self.transform = transform
        # self.transform_mask = transform_mask
        print(f'length of the dataset : {len(self.total_imgs)}')
...
class MOVI_E(Dataset):
    def __init__(self, root, img_size, num_segs, transform, transform_mask=None, split='train'):
        ...
        self.transform = transform
        # self.transform_mask = transform_mask
        # a1 = [self.__getitem__(i)['input'].max() for i in range(10000)]
    ...
    def __getitem__(self, idx):
        ...
        return {'input':img, 'fname':str(self.total_imgs[idx])}
        ...
        #         mask = Image.open(os.path.join(self.root, self.total_imgs[idx].replace('image', f'mask_{i:02}'))).convert('1')
        #         masks.append(mask)
