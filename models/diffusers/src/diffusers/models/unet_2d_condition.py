...
class UNet2DConditionModelWithPE(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
    UNet2DConditionModel is a conditional 2D UNet model that takes in a noisy sample, conditional state, and a timestep
    and returns sample shaped output.
    ...
    def __init__(
        ...
        elif mid_block_type == "UNetMidBlock2DCrossAttnWithPE":
            self.mid_block = UNetMidBlock2DCrossAttnWithPE(
                in_channels=block_out_channels[-1],
                temb_channels=time_embed_dim,
                resnet_eps=norm_eps,
                resnet_act_fn=act_fn,
                ...
                use_linear_projection=use_linear_projection,
                upcast_attention=upcast_attention,
                sample_size=sample_size//2**(len(up_block_types)-1),
            )
        elif mid_block_type == "UNetMidBlock2DSimpleCrossAttn":
            self.mid_block = UNetMidBlock2DSimpleCrossAttn(
                in_channels=block_out_channels[-1],
        ...
        for i, up_block_type in enumerate(up_block_types):
            ...
            up_block = get_up_block(
                ...
                upcast_attention=upcast_attention,
                resnet_time_scale_shift=resnet_time_scale_shift,
                sample_size=sample_size//max(2**(len(up_block_types)-i-1), 1),
            )
            self.up_blocks.append(up_block)
            prev_output_channel = output_channel
