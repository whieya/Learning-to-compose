...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model, num_heads, dropout=0., gain=1., vis_attn=False):
        super().__init__()
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, max_len, d_model, dropout=0.1, trunc_emb=False):
        super().__init__()
        self.dropout = nn.Dropout(dropout)
        self.pe = nn.Parameter(torch.zeros(1, max_len, d_model), requires_grad=True)
        self.trunc_emb=trunc_emb
...
class TransformerDecoderBlock(nn.Module):
    ...
    def __init__(self, max_len, d_model, num_heads, dropout=0., gain=1., is_first=False, causal_mask=False):
        super().__init__()
        ...
        self.encoder_decoder_attn = MultiHeadAttention(d_model, num_heads, dropout, gain, vis_attn=True)
    ...
    def forward(self, input, encoder_output):
        ...
        x, dec_mask = self.encoder_decoder_attn(x, encoder_output, encoder_output)
        input = input + x
        
        x = self.ffn_layer_norm(input)
        x = self.ffn(x)
        return input + x, dec_mask



class TransformerDecoder(nn.Module):
    
    def __init__(self, num_blocks, max_len, d_model, num_heads, dropout=0., causal_mask=False):
        super().__init__()
        
        if num_blocks > 0:
            gain = (3 * num_blocks) ** (-0.5)
            self.blocks = nn.ModuleList(
                [TransformerDecoderBlock(max_len, d_model, num_heads, dropout, gain, is_first=True, causal_mask=causal_mask)] +
                [TransformerDecoderBlock(max_len, d_model, num_heads, dropout, gain, is_first=False, causal_mask=causal_mask)
                 for _ in range(num_blocks - 1)])
        else:
            self.blocks = nn.ModuleList()
    ...
    def forward(self, input, encoder_output):
        ...
        """
        for block in self.blocks:
            input, attns = block(input, encoder_output)
        
        return self.layer_norm(input), attns.mean(1)
